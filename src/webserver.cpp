...
int on_url(http_parser* parser, const char* header, long unsigned int size){
    ...
    sprintf(my_data->url, "%.*s", (int)size, header);
    ...
    do{
        ...
        if(nparsed != nrecvd){
            ...
            if(parser->method == 1)
            {
               ...
               // char file_path[255];
               // make_path_from_url(my_data->url, file_path);
                char* result = (char*)malloc(sizeof(RESPONSE_OK)+ sizeof(CONTENT));
